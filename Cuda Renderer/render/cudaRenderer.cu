...
shadePixel(float2 pixelCenter, float3 p, float4 *imagePtr, int circleIndex) {
    ...
    // Global memory write
    *imagePtr = newColor;
    //printf("imagePtr: %f,%f,%f,%f", newColor.x, newColor.y, newColor.z, newColor.w);
...
__global__ void kernelRenderPixels() {
    ...
    if (index >= cuConstRendererParams.numberOfPixels)
        return;

    //printf("this is KernelRenderPixels!!!");
    ...
    //    if (pixelX==3) {
    //        printf("index:%d, pixel X:%d , pixel Y:%d, numberOfCircles: %d,imageWidth:%d\n", index, pixelX, pixelY,
    //               numberOfCircles, imageWidth);
    //    }
    ...
    for (int i = 0; i < numberOfCircles; i++) {
        ...
        //printf("here *******2\n");
        imgPtr = (float4 * )(&cuConstRendererParams.imageData[4 * (index)]);
        //printf("here *******3\n");
...
void CudaRenderer::setup() {
    ...
    params.numberOfPixels = image->width * image->height;
...
__global__ void kernelRenderPerBlock() {
    ...
    int blockL = blockCol * BOX_WIDTH;
    int blockB = blockRow * BOX_WIDTH;
    int blockR, blockT;
    if (blockCol != gridDim.x - 1)
        blockR = (blockCol + 1) * BOX_WIDTH;
    else
        blockR = imageWidth;
    if (blockRow != gridDim.y - 1)
        blockT = (blockRow + 1) * BOX_WIDTH;
    else
        blockT = imageHeight;

    float blockPosL = invWidth * (static_cast<float>(blockL) + 0.5f);
    float blockPosR = invWidth * (static_cast<float>(blockR - 1) + 0.5f);
    float blockPosB = invHeight * (static_cast<float>(blockB) + 0.5f);
    float blockPosT = invHeight * (static_cast<float>(blockT - 1) + 0.5f);
    ...
    __shared__ uint circleIndice[BLOCKSIZE];

    int circleId;
    int index3;
    float3 p;
    float rad;
    for (int base = 0; base < numberOfCircles; base += SCAN_BLOCK_DIM) {
        circleId = base + linearThreadIndex;
        if (circleId < numberOfCircles) {
            index3 = 3 * circleId;
            p = *(float3 * )(&cuConstRendererParams.position[index3]);
            rad = cuConstRendererParams.radius[circleId];
            prefixSumInput[linearThreadIndex] = circleInBox(p.x, p.y, rad, blockPosL, blockPosR, blockPosT, blockPosB);
        } else {
        ...
        if (prefixSumInput[linearThreadIndex] == 1)
            circleIndice[prefixSumOutput[linearThreadIndex]] = linearThreadIndex;

        __syncthreads();

//        int circleLast;
//        if(numberOfCircles-base>SCAN_BLOCK_DIM){
//            circleLast=SCAN_BLOCK_DIM-1;
//        }else{
//            circleLast=(numberOfCircles-1)%SCAN_BLOCK_DIM;
//        }
//        int circleLen = prefixSumOutput[circleLast]+prefixSumInput[circleLast];

        int circleLen = prefixSumOutput[SCAN_BLOCK_DIM - 1] + prefixSumInput[SCAN_BLOCK_DIM - 1];

        if (circleLen > 0) {
            if (pixelX < imageWidth && pixelY < imageHeight) {

                for (int i = 0; i < circleLen; i++) {
                    circleId = base + circleIndice[i];
                    index3 = 3 * circleId;
                    p = *(float3 * )(&cuConstRendererParams.position[index3]);
//                    printf("circleId, p.x=%f, p.y=%f, p.z=%f, pixelCenterNorm.x=%f, pixelCenterNorm.y=%f\n", circleId,
//                           p.x,
//                           p.y, p.z, pixelCenterNorm.x, pixelCenterNorm.y);
                    shadePixel(pixelCenterNorm, p, &tmpImgPtr, circleId);
                }
            }
        }
        __syncthreads();
...
void CudaRenderer::render() {
    // 256 threads per block is a healthy number
    int blockARow = (image->width + BOX_WIDTH - 1) / BOX_WIDTH;

    dim3 blockDim(BOX_WIDTH, BOX_WIDTH);
    dim3 gridDim(blockARow, blockARow);
    printf("image width: %d, image height: %d\n", image->width, image->height);

    kernelRenderPerBlock<<<gridDim, blockDim>>>();
